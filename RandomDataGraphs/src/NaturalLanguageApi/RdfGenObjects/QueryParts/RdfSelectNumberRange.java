...
/**
 * Class that represents the mode in which we select a number of results (from a range) in RDF format
 */
public class RdfSelectNumberRange extends RdfQueryResultSelection {
     ...
     */
    public RdfSelectNumberRange(int min, int max) {
        _minValue = min;
        _maxValue = max;
    }
    ...
    public String toRdfString(int tabOffset) {
        ...
        lines.add(maxVal);
        selectRange += RdfPrinter.printTabbedComposite(tabOffset, RdfGenTypes.RDF_GEN_PREFIX + RdfGenTypes.RES_RDF_TYPE_SELECT_NUMBER_RANGE, lines);
        return selectRange;
    }
}